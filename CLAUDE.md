...
- Crypto: libsodium via `sodium` Dart package v4 (Ed25519 identity, XChaCha20-Poly1305 feed encryption, BLAKE2b hashing). MegOLM-shaped per-message keys: each post's AEAD key is derived as `BLAKE2b-256(chainRoot || "finch-msg-key-v1" || u64_be(msgSeq))`. Wire format carries `epoch` + `msg_seq` on every `EncryptedEvent`. Receivers archive prior chain roots in `follow_feed_key_history` so cached content stays decryptable across rotations. Derivation is currently flat (no within-epoch ratchet); a forward-secrecy upgrade swaps the derivation only, no wire change.
