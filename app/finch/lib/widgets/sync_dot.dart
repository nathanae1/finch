...
class _SyncDotState extends State<SyncDot> with SingleTickerProviderStateMixin {
  ...
  Color _colorFor(FinchTheme finch) => switch (widget.state) {
        SyncState.synced => finch.colors.success,
        SyncState.syncing => finch.colors.sage,
        SyncState.waiting => finch.colors.clay,
        SyncState.offline => finch.colors.stone,
      };

  @override
  Widget build(BuildContext context) {
    final finch = FinchTheme.of(context);
    final color = _colorFor(finch);
