...
private let kBackgroundSyncIdentifier = "dev.finch.app.background_sync"
private let kBackgroundProcessingIdentifier = "dev.finch.app.background_processing"
private let kBackgroundChannelName = "dev.finch.app/background_sync"
...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  private func scheduleBackgroundTasks() {
    ...
    do {
      ...
      NSLog("[finch.bg] failed to submit BGAppRefreshTask: \(error)")
    }
    ...
    do {
      ...
      NSLog("[finch.bg] failed to submit BGProcessingTask: \(error)")
    }
  }

  private func handleAppRefreshTask(task: BGAppRefreshTask) {
    NSLog("[finch.bg] BGAppRefreshTask fired")
    // Re-submit next request first so a failure here doesn't break the
    // recurring schedule.
    scheduleAppRefresh()

    task.expirationHandler = {
      NSLog("[finch.bg] BGAppRefreshTask expiration handler — bailing")
  ...
  private func handleProcessingTask(task: BGProcessingTask) {
    NSLog("[finch.bg] BGProcessingTask fired")
    scheduleProcessing()

    task.expirationHandler = {
      NSLog("[finch.bg] BGProcessingTask expiration handler — bailing")
      task.setTaskCompleted(success: false)
    }
  ...
  private func invokeDartBackgroundSync(
    ...
    guard let channel = backgroundChannel else {
      NSLog("[finch.bg] backgroundChannel not available — Dart engine not initialised")
      completion(false)
      return
    }
    channel.invokeMethod("runBackgroundSync", arguments: ["allowTor": allowTor]) { result in
      if let error = result as? FlutterError {
        NSLog("[finch.bg] Dart sync error: \(error.message ?? "")")
        completion(false)
      } else if let success = result as? Bool {
        completion(success)
      } else {
        NSLog("[finch.bg] Dart sync returned unexpected result: \(String(describing: result))")
        completion(false)
      }
    }
  }
